...
public class Server{
    ...
    try {
       ...
       while ( activitateClient ) 
             {    
                ...
                mesajDescompus = mesaj.split(" ",3);
                switch( mesajDescompus[0] )
                      {
                        case "LIST" : list();
                                      break;
                        case "NUME" : if( mesajDescompus[1] != null )
                                         schimbareNume(mesajDescompus[1]);
                                      else outputClient.writeUTF( "SERVER Nu ai dat un nume\n" );
                                      break;
                        case "BCAST" : if( mesajDescompus.length == 3 )
                                          broadcast(mesajDescompus[1] + " " + mesajDescompus[2] );
                                       else if( !mesajDescompus[1].equals("") )  
                                               broadcast(mesajDescompus[1]);
                                            else outputClient.writeUTF( "SERVER Nu ai dat un mesaj\n" );
                                       break;
                        case "QUIT" : quit();                   
                                      break;   
                        case "MSG" :  if( !msg( mesajDescompus[1] , mesajDescompus[2] ) )
    ...
    synchronized void schimbareNume( String numeNou ){
      ...
      try{
           ...
           else outputClient.writeUTF( "SERVER Numele " + numeNou + " este folosit\n" );
    ...
    synchronized Boolean msg( String nume , String mesaj ){
  
      int indexNume = numeUtilizatori.indexOf( nume );
      if( indexNume != -1 && mesaj != null )
         try{
              Sockete.get(indexNume).writeUTF( "MSG " + nume + " : " + mesaj );
              return true;
        }catch(IOException e){
           try {
                outputClient.writeUTF( "MSG " + nume + " s-a deconectat" );
           }catch (IOException ex) {}
           return true;
             ...
             {
                 try {
                       outputClient.writeUTF( "MSG " + nume + " nu e online" );
                 }catch (IOException ex) {}
                 return true;
             }
           else return false;
